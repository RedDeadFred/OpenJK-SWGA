					...
					if ( !FS_FilenameCompare( pakFile->name, filename ) ) {
						...
						if ( !(pak->referenced & FS_GENERAL_REF)) {
							if( !FS_IsExt(filename, ".shader", l) &&
							    ...
							    Q_stricmp(filename, "swgampgamex86.dll") != 0 &&
						...
						{
							if ( Q_stricmp( filename, "swgacgame.qvm" ) == 0 ||
									Q_stricmp( filename, "swgacgamex86.dll" ) == 0 )
							{
						...
						{
							if ( Q_stricmp( filename, "swgaui.qvm" ) == 0 ||
									Q_stricmp( filename, "swgauix86.dll" ) == 0 )
							{
...
void FS_Startup( const char *gameName ) {
	...
	fs_gamedirvar = Cvar_Get ("fs_game", "SWGA", CVAR_INIT|CVAR_SYSTEMINFO, "Mod directory" );
